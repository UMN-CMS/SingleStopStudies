  ...
  \begin{columns}[t]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \coloreditem{early} Early Run3 data.
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
...
\begin{frame}[label=regions]{General Analysis Features}
  ...
  \item Multiple b-jets, and large angular separation between b-jets. 
  \end{itemize}
  ...
  \begin{center}
    \scalebox{0.8}{
      \begin{tabular}{|ccccc|}
        ...
        \multicolumn{5}{|c|}{$\mathrm{N}_e , \mathrm{N}_\mu  = 0$} \\
        \multicolumn{5}{|c|}{$\Delta R_{\mathrm{b_1,b_2}} > 1$ (SR Only)} \\ 
        \multicolumn{5}{|c|}{\rule[-0.5em]{0em}{0em}$m_4 \equiv m_{\mathrm{j_1,j_2,j_3,j_4}}$} \\ \hline
        \specialcell{$\lambda_{312}$\\ Uncompressed SR}
        ...
        $\mathrm{N_{b,m} } \geq 2$ & $\mathrm{N_{b,m} } \geq 2$ & $\mathrm{N_{b,t} } \geq 3$ & $\mathrm{N_{b,t}} \geq 3$ & $\mathrm{N_{b,l}} = 0$ \\
...
\begin{frame}{Resonance Reconstruction}
  ...
  \begin{center}
    \begin{onlyenv}<1>
      \scalebox{0.6}{\includestandalone{\commonfiles{general/two_peaks}}}
    \end{onlyenv}
    \begin{onlyenv}<2->
...
\begin{frame}{Representing Histograms With Gaussians}
  ...
  \item The answer: consider each of the N bins to be a random variable: part of a N-Multivariate Normal (MVN).
...
\begin{frame}{Kernels and Scales Continued}
  ...
  \begin{onlyenv}<2>
    ...
    \posannot[210:3cm]{ls2}{fill=UMNMaroon!10, draw=UMNMaroon}{Long length scales can't fit the data.}
  \end{onlyenv}
\end{frame}
